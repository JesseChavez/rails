...
class FinderTest < ActiveRecord::TestCase
  ...
  def test_take_and_first_and_last_with_integer_should_use_sql_limit
    assert_sql(/LIMIT|ROWNUM <=|FETCH (FIRST|NEXT)/) { Topic.take(3).entries }
    assert_sql(/LIMIT|ROWNUM <=|FETCH (FIRST|NEXT)/) { Topic.first(2).entries }
    assert_sql(/LIMIT|ROWNUM <=|FETCH (FIRST|NEXT)/) { Topic.last(5).entries }
  end
